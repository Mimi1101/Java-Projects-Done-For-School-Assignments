...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E> {
    ...
    private int count;
    /**
     * A comparator lambda function that compares two elements of this AVL tree;
     * cmp.compare(x,y) gives 1. negative when x less than y 2. positive when x
     * greater than y 3. 0 when x equal y
     */
    ...
    public boolean isFibonacci() {
        int heightOfTheTree = this.height();
        int fibonnaciOrNot = fibonacci(heightOfTheTree +2) -1;
        if(fibonnaciOrNot == this.fullCount()){
            return true;
        }
        return false;
    }
    ...
    /**
     * An auxiliary method that inserts a new node in the tree or updates a node
     * if the data is already in the tree.
     *
     * @param curRoot a root of a subTree
     * @param newNode the new node to be inserted
     * @param taller indicates whether the subTree becomes taller after the
     * insertion
    ...
    /**
     * An auxiliary method that in-order traverses the subTree at the specified
     * node
     *
    ...
    /**
     * Iteratively determines the height of the subTree rooted at the specified
     * node by only examining nodes along its longest path
     *
    ...
    /**
     * An auxiliary function that recursively counts the number of full nodes in
     * this tree
     *
     * @param node the root of a subTree
     * @return the number of full nodes in the subTree rooted at the specified
     * node
     */
     ...
     * @param paths an array list of strings representing root to leaf paths in
    ...
    /**
     * An auxiliary function that iteratively computes the nth Fibonacci number
     *
